 ...
 */
public final class RPGL extends AbstractPlanner {

    /**
     * The logger of the class.
     */
    private static final Logger LOGGER = LogManager.getLogger(RPGL.class);
    ...
    /**
     * Creates a new RPGL planner with the default parameters.
     */
    public RPGL() {
        super();
        this.setHeuristicType(RPGL.DEFAULT_HEURISTIC);
        this.setWeight(RPGL.DEFAULT_WEIGHT);
        this.setSaveState(RPGL.DEFAULT_STATISTICS);
    }
    ...
    public SequentialPlan search(final CodedProblem problem) {
        ...
        while (!open.isEmpty() && plan == null && time < timeout) {
            ...
            BitState inita = new BitState(problem.getGoal());
            System.out.println("GOOOOOAL " + inita);

            // If the goal is satisfy in the current node then extract the search and return it
            if (current.satisfy(problem.getGoal())) {
                plan = this.extract(current, problem);
                System.out.println("asdasdsad " + current.satisfy(problem.getGoal()));
            } else {
    ...
    /**
     * The main method of the <code>RPGL</code> example. The command line syntax is as follow:
     * <p>
     * <pre>
     * usage of RPGL:
     *
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            final Properties arguments = AbstractPlanner.parseArguments(args, LOGGER, RPGL.getDefaultArguments());
    ...
    private static Properties getDefaultArguments() {
        ...
        options.put(AbstractPlanner.Argument.HEURISTIC, RPGL.DEFAULT_HEURISTIC);
        options.put(AbstractPlanner.Argument.WEIGHT, RPGL.DEFAULT_WEIGHT);
        options.put(AbstractPlanner.Argument.TIMEOUT, RPGL.DEFAULT_TIMEOUT * 1000);
        options.put(AbstractPlanner.Argument.TRACE_LEVEL, RPGL.DEFAULT_TRACE_LEVEL);
        options.put(AbstractPlanner.Argument.STATISTICS, RPGL.DEFAULT_STATISTICS);
        return options;
    }
}
